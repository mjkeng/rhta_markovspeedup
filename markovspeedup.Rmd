...
calculate_bootstrap_mean <- function(data, n, replacement){
...
for(i in c(1:500)){
  set.seed(i)
  boot_mean[i] <- calculate_bootstrap_mean(
...
- Temporary variable `i` created and gets updated in the global environment 
  - For more complex code, you may be left with several temporary variables in the global environment 
...
- You can "vectorise" iterations (i.e. your loops) using functions from the `apply()` family<sup>†</sup>
...
for(i in c(1:500)){
  set.seed(i)
  boot_mean[i] <- calculate_bs_mean(data = df$cost, n = n_sample, replacement = TRUE)
}
...
.pull-right[
```{r eval = TRUE}
boot_mean <- sapply(c(1:500), function(i){
  set.seed(i)
...
.footnote[
[†] Read more about the `apply()` family of functions in [Carlo Fanara | Datacamp tutorial on the R Apply Family](https://www.datacamp.com/community/tutorials/r-tutorial-apply-family). Vectorised iterations can also be implemented using the `map()` family of functions in [`purrr`](https://purrr.tidyverse.org/). More details in [Garrett Grolemund & Hadley Wickham | R for Data Science](https://r4ds.had.co.nz/iteration.html)
]
...
- Running several processes simulataneously on multiple processors/cores of the computer
...
Notice that parallelising here is actually _SLOWER_? 
...
Consider a simple simulation model where you are predicting the state a patient is in, and performing costs and QALYs calculation in each cycle. 
...
df_median_runtime <- readRDS("src/df_benchmark.rds") %>% 
  ...
  select(name, median) %>% 
  slice(match(c("base", "vectorisetx", "vectorisesmp", "parallelisesmp", "vectorisetx_rcppcycle"), name))

# Pre-define column names
colname_runtime <- c("Code", "Median time across 20 iterations")
...
df_median_runtime %>% 
  ...
  kable(col.names = colname_runtime, digit = 2) %>% 
  kable_styling() 
...
df_median_runtime %>% 
  filter(name %in% c("base", "vectorisetx", "vectorisesmp", "parallelisesmp")) %>% 
  kable(col.names = colname_runtime, digit = 2) %>% 
  kable_styling() 
...
df_median_runtime %>% 
  filter(name %in% c("base", "vectorisetx", "vectorisesmp", "parallelisesmp", "vectorisetx_rcppcycle")) %>% 
  kable(col.names = colname_runtime, digit = 2) %>% 
  kable_styling() 
...
Vectorising / parallelising etc.

--

## What should you optimise? 

Profile your code to find bottlenecks<sup>†</sup>.

.footnote[

[†] One way to easily profile your code is with [`profvis`](https://rstudio.github.io/profvis/). More about code profiling in [Hadley Wickham | Advanced R](https://adv-r.hadley.nz/perf-measure.html)
...
<blockquote><i>
We should forget about small efficiencies, say about 97% of the time: premature optimization is the root of all evil. -- DonaldKnuth 
</i></blockquote>
