...
bs_parallelise <- parallelsugar::mclapply(c(1:500), function(i){
  calculate_bs_mean(data = df$cost, n = n_sample, replacement = TRUE)
}) %>% unlist()
...
```
Median time taken = `r round(bench::mark(parallelsugar::mclapply(c(1:500), function(i){set.seed(i); calculate_bs_mean(data = df$cost, n = n_sample, replacement = TRUE)}, mc.cores=2))$median*1000,2)`ms
...
```{r setup-markov-time, eval = TRUE, echo = FALSE, cache = TRUE}
...
func_call <- c(
  ...
  "markov_expanded_parallisesmp_mclapply(n.states = 10, n.cycles = 100, n.samples = 25000)"
)
...
for(i.treatment in c(1:n.treatments)){
  ...
  mclapply(c(1:n.samples), function(i.sample){ #<<
